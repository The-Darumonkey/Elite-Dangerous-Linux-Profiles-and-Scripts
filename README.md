...
After losing scripts and other useful bits of Elite-related stuff during a reinstall (yes, I am an idiot), I decided to create this repository so I don't have to rewrite them all next time. My other repository is a personal fork of Input Remapper that spawns several virtual gamepads instead of just one, enabling multi-stick (HOSAS) setup with response curves. This repo holds the Input Remapper profiles for my devices, my custom `.binds` file for Elite: Dangerous, and scripts for joystick wrangling with `evdev-joystick`. I reasoned that I might as well make the repo public as the files and info contained here could be useful to others.
...
Make sure to change the event ID to the one associated with your device (displayed when runnning `evtest`). The `evdev-joystick` command above sets the deadzone (`--d`, also known as `flat` in `evtest`) and fuzz (`--f`) values to zero. You may find some benefit in experimenting with the fuzz value if you have, say, a Thrustmaster T.16000M with a dodgy potentiometer on the yaw axis. I use Input Remapper to implement a response curve on my joystick and the stick's axis sensors are all Hall effect, so I need neither a deadzone nor filtering for noisy pots, hence the zeros.

While the approach of using `evdev-joystick` in the terminal works, it is suboptimal in that you must run it every time you plug in the joystick or turn on your PC, which gets old pretty quickly. The `joystick-init.sh` script automatically sets both the `deadzone (flat)` and `fuzz` values to zero. It uses `by-id` symlinks to sidestep the issue where event IDs can - and do - often change. 
...
I can't vouch that these profiles will work on another machine, even if you have the exact same input device - I've never tried. If they don't work, you might still find it useful to inspect them to inform your choices when setting up your own device profiles. Remember that they depend on a modified fork of Input Remapper (in my other repo) that spawns 3 virtual gamepads instead of the usual 1. 

These `.json` files live in the respective folder for each device in `/home/<username>/.config/input-remapper-2/presets/`

### Profile details
1. `Elite-SpacePilot` is a profile for the (ancient but awesome!) 3DConnexion SpacePilot Pro. It maps 5 of the axes on the 6-axis puck to gamepad analogue axes, and the buttons to keystrokes. The 6th axis - rX (X-axis rotation) is not used in Elite: Dangerous' galaxy map. You could, however, always map it to a gamepad axis and then map _that_ axis to _some_ analogue control in the game, such as FSS frequency or galaxy map zoom. The (lack of) mapping reflects only my own personal preference. 
...
- If you are going to use this profile, I suggest starting with a blank `.binds` in Elite and binding the primary trigger, the secondary trigger and then the hats. It doesn't really matter what you do with the hats; they're all mapped to unique keystrokes. You might run into a few conflicts when mapping the full-spectrum system scanner (FSS) and detailed surface scanner (DSS). I will leave it to you to work that out to your preference. 

## Custom.4.2.binds
The `.binds` file is only really included for when (not if!) I have to reinstall. You may find something useful when inspecting them, I suppose. The bindings make no sense at all if you're actually using a keyboard for important ship functions. They are the way they are because I don't care what key I'm "pressing" when it's mapped to a joystick button emulating a keypress through Input Remapper. 
